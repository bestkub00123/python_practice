...
for p in type_s:
    if 'k' in p:
        type_s[type_s.index(p)] = str(int(p.replace('kb',''))*(10**3))+'b'       
    if 'm' in p:
        type_s[type_s.index(p)] = str(int(p.replace('mb',''))*(10**6))+'b'
...
"""for q in range((len(collect))):
    output_s.append([collect[q],type_s[q]])"""